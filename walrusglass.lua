...
if searchArgs("root") then
	...
	for k,v in pairs(fs.list("/plasticApps")) do
		if not(fs.isDir("/plasticApps/"..v)) then
			local f = io.open("/plasticApps/"..v, "r") 
...
local function runPlastic()
	...
	if fs.exists("/plasticOptions") and not(fs.isDir("/plasticOptions")) then
		local f = io.open("/plasticOptions", "r")
		local data = f:read("*a")
	...
	if not(settings) or not((type(settings["name"]) == "string") and (type(settings["use12hour"]) == "boolean") and
		...
		closeAnimation()
		reliableSleep(1)
		shell.run(plasticInstallation, "nodaemon", unpack(tArgs))
		error()
	end
	...
	local function getWT(city, canceller)
		local function getRawWT(city, canceller) --f or c for unit
			...
			if canceller then
				resp, webData = plasticGet("http://api.worldweatheronline.com/free/v1/weather.ashx?q="..textutils.urlEncode(city).."&format=xml&extra=localObsTime&num_of_days=1&includelocation=yes&key=uyp4r5ekjns64pmpvv3qkynj", true)
			else
				resp, webData = plasticGet("http://api.worldweatheronline.com/free/v1/weather.ashx?q="..textutils.urlEncode(city).."&format=xml&extra=localObsTime&num_of_days=1&includelocation=yes&key=uyp4r5ekjns64pmpvv3qkynj")
			end
			if resp == "cancel" then
				return "cancel"
	...
	local function mainThread()	
		local function start()
			...
			if firstTime then
				...
				local f = io.open("/plasticOptions", "w")
		...
		local function convert(query)
			...
			local resp, msg = plasticGet("http://www.stands4.com/services/v2/conv.php?uid=2464&tokenid=lQAygI15b9x34e2L&expression=" .. textutils.urlEncode(query))
			if resp == "success" then
				...
				if e == "chat_command" then
					...
					if not(skipAll) then
						...
						elseif msg == "update" then
							...
							local resp, data = plasticGet("http://pastebin.com/raw.php?i=43nHyKXU")
							if resp == "success" then
								local f = io.open(plasticInstallation, "w")
								f:write(data)
								f:close()
								closeAnimation()
								reliableSleep(1)
	...
	elseif eResp then
		if not(fs.exists("/plasticLog")) then
			local f = io.open("/plasticLog", "w") f:write("-- Plastic Error Logs --\n") f:close()
		end
		local f = io.open("/plasticLog", "a")
		f:write(eResp .. "\n")
		f:close()
		mainText.setText("")
		secondText.setText("")
...
if not(searchArgs("nodaemon")) then
	parallel.waitForAny(plasticWrapper, function() shell.run("/rom/programs/shell") end)
	closeAnimation()
	term.clear()
	term.setCursorPos(1,1)
